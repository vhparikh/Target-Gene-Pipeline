...
y_matrix = pd.read_csv('PIK3CA_y_matrix.tsv', delimiter='\t')
samples = x_matrix['SAMPLE_BARCODE']
diseases = y_matrix['DISEASE']
x_matrix['DISEASE'] = diseases

unique_diseases = x_matrix['DISEASE'].unique()
diseases_encoded = []
for disease in diseases:
    diseases_encoded.append(list(unique_diseases).index(disease))
print(unique_diseases)
print(diseases_encoded)
...
combined = x_matrix.drop(columns=['SAMPLE_BARCODE', 'log10_mut', 'DISEASE'])
...
pca_df = pd.DataFrame(pca_data, index=[*diseases], columns=labels)

plt.scatter(pca_df.PC1, pca_df.PC2, c=colormap[diseases_encoded])
...
loading_scores = pd.Series(pca.components_[0], index=diseases)
